// Seed data used as a fallback when Supabase isn't configured (e.g. local
// development without env vars). In production these arrays are never
// rendered — see employeeStore, useTimeEntries, useAnnouncements for how
// they're conditionally loaded.

import { Employee, TimeEntry, Announcement, OvertimeConfig } from '@/types/workforce';
...
/** ISO timestamp when the employee's current break/lunch started (if any). */
export function getActiveBreakStart(employeeId: string, entries: TimeEntry[]): string | undefined {
  const entry = entries.find(e => e.employeeId === employeeId && !e.clockOut);
  if (!entry) return undefined;
  const activeBreak = entry.breaks.find(b => !b.endTime);
  return activeBreak?.startTime;
}
